...
// 0. Máximo valor general de la matriz de entrada
const maxVal = computed(() => {
  if (!props.matrix.length) return 0
  let max = -Infinity
...
// 1. Matriz Convertida: (M_max - C_ij)
const convertedMatrix = computed(() => {
  return props.matrix.map(row => 
    row.map(val => maxVal.value - val)
  )
})

// 2. Alfa (α): Mínimo de cada fila de la matriz convertida
const alpha = computed(() => {
  return convertedMatrix.value.map(row => Math.min(...row))
})

// 3. Matriz reducida por Filas
const rowReducedMatrix = computed(() => {
  return convertedMatrix.value.map((row, i) => 
    row.map(val => val - alpha.value[i])
  )
})

// 4. Beta (β): Mínimo de cada columna sobre la matriz reducida por filas
const beta = computed(() => {
...
<template>
  <div class="algorithm-container">
    <!-- Banner explicativo de conversión -->
    <div class="conversion-banner">
      <span class="banner-title">Paso 0: Conversión por Maximización</span>
      <p class="banner-text">
        Elemento Máximo (M) = <strong>{{ maxVal }}</strong>. Matriz Invertida = <code>{{ maxVal }} - C<sub>ij</sub></code>
      </p>
    </div>
...
.conversion-banner { background-color: #ecfdf5; border-left: 4px solid #10b981; padding: 0.5rem 0.75rem; border-radius: 0.375rem; }
.banner-title { font-size: 0.75rem; font-weight: 700; color: #065f46; display: block; }
.banner-text { font-size: 0.7rem; color: #047857; margin: 0.2rem 0 0 0; }
